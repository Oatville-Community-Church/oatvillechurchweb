...
function staticCopyPlugin() {
  return {
    ...
    generateBundle() {
      ...
      // Copy live status snapshot generated by GitHub Actions (if present).
      const liveStatusFile = path.resolve(__dirname, 'src', 'data', 'live-status.json');
      if (fs.existsSync(liveStatusFile)) {
        this.emitFile({ type: 'asset', fileName: 'data/live-status.json', source: fs.readFileSync(liveStatusFile) });
      }
...
Technologies: Vite, Tailwind CSS, PWA`;
  ...
  outDir: path.resolve(__dirname, 'dist'),
    ...
    plugins: [
      ...
      VitePWA({
        ...
        workbox: {
          // Keep YouTube RSS network-fresh; stale cache here causes old videos/streams.
          globIgnores: ['**/data/you-tube-rss.xml', '**/data/live-status.json'],
          runtimeCaching: [
            {
              urlPattern: /\/data\/you-tube-rss\.xml(\?.*)?$/,
              handler: 'NetworkFirst',
              options: {
                cacheName: 'youtube-rss-runtime',
            ...
            {
              urlPattern: /\/data\/live-status\.json(\?.*)?$/,
              handler: 'NetworkFirst',
              options: {
                cacheName: 'youtube-live-status-runtime',
                networkTimeoutSeconds: 8,
                expiration: {
                  maxEntries: 4,
                  maxAgeSeconds: 60 * 60
